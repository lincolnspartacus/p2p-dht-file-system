    ...
    def getNode(self, request, context):
        ...
        for key in keyList:
            ...
            try:
                ...
                print(f'[tracker] Inside getNode = cant reach node. Delete entry = ({key})')
                self.gcLock.acquire()
                self.gccollect.append(key) 
                self.gcLock.release()


        self.gcLock.acquire()
        self.tbLock.acquire()
        for key in self.gccollect:
            self.table.pop(key,None)
        self.tbLock.release()
        self.gccollect.clear()
        self.gcLock.release()
